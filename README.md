...
- `src/pages/` contains the reference application pages. Page guides are grouped by area; demo-only helpers live in `src/pages/demos/`.
- `src/routes/route-manifest.ts` owns the route table. `src/config/site-navigation.tsx` owns shared site navigation, and `src/config/component-navigation.ts` owns component-area navigation.
