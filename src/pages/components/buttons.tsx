            ...
            <div className="space-y-14 px-4 pb-12 pt-6 sm:px-6 lg:px-8">
              ...
              <section className="space-y-5" aria-labelledby="buttons-examples-heading">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <article className="space-y-3 rounded-lg border bg-card p-4"><div><h3 className="text-lg font-semibold">Action hierarchy</h3><p className="mt-1 text-sm text-muted-foreground">Visual emphasis should match the consequence and frequency of the action.</p></div><div className="flex flex-wrap gap-3 rounded-md border bg-muted/20 p-4"><button type="button" className="rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Save changes</button><button type="button" className="rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Cancel</button><button type="button" className="rounded-md border border-destructive px-3 py-2 text-sm text-destructive focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Delete draft</button></div><div className="space-y-1 text-sm"><p><strong>Use when:</strong> one primary action needs clear emphasis and secondary actions should remain available.</p><p><strong>Avoid when:</strong> every action is styled as primary or destructive styling is used for ordinary cancellation.</p><p><strong>Check:</strong> labels describe outcomes and the destructive action is not distinguished by color alone.</p></div></article>
                  <article className="space-y-3 rounded-lg border bg-card p-4"><div><h3 className="text-lg font-semibold">Icon and text</h3><p className="mt-1 text-sm text-muted-foreground">Icons reinforce a familiar action; they do not replace a clear label when the action is unfamiliar.</p></div><div className="flex flex-wrap gap-3 rounded-md border bg-muted/20 p-4"><button type="button" className="rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">＋ Add member</button><button type="button" aria-label="Open settings" className="rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">⚙</button></div><div className="space-y-1 text-sm"><p><strong>Use when:</strong> the icon is recognizable and improves scanning.</p><p><strong>Avoid when:</strong> an icon-only action could be misunderstood or its meaning changes by context.</p><p><strong>Check:</strong> icon-only buttons have an accessible name and a touch-sized target.</p></div></article>
                  <article className="space-y-3 rounded-lg border bg-card p-4"><div><h3 className="text-lg font-semibold">Loading and disabled</h3><p className="mt-1 text-sm text-muted-foreground">State changes should explain whether work is in progress or the action is unavailable.</p></div><div className="flex flex-wrap gap-3 rounded-md border bg-muted/20 p-4"><button type="button" disabled aria-disabled="true" className="cursor-not-allowed rounded-md border px-3 py-2 text-sm opacity-60">Save unavailable</button><button type="button" aria-busy="true" className="rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Saving…</button></div><div className="space-y-1 text-sm"><p><strong>Use when:</strong> disabled means the action cannot be used in the current context, or loading prevents duplicate work.</p><p><strong>Avoid when:</strong> disabling hides the reason a person cannot proceed or substitutes for read-only content.</p><p><strong>Check:</strong> the implementation communicates busy state, preserves the action’s name, and restores focus appropriately.</p></div></article>
                  <article className="space-y-3 rounded-lg border bg-card p-4"><div><h3 className="text-lg font-semibold">Submit versus navigation</h3><p className="mt-1 text-sm text-muted-foreground">Use a button for an action and a link for a destination.</p></div><div className="flex flex-wrap gap-3 rounded-md border bg-muted/20 p-4"><form className="flex flex-wrap gap-3"><label className="sr-only" htmlFor="button-example-title">Title</label><input id="button-example-title" className="rounded-md border px-3 py-2 text-sm" placeholder="Title" /><button type="submit" className="rounded-md bg-primary px-3 py-2 text-sm text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Create</button></form><a href="#button-example-details" className="rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">View details</a></div><div className="space-y-1 text-sm"><p><strong>Use when:</strong> submission changes data; use a link when activation changes location.</p><p><strong>Avoid when:</strong> styling a link as a button only because it looks prominent.</p><p><strong>Check:</strong> native keyboard behavior, form submission, and destination announcements remain intact.</p></div></article>
                  <article className="space-y-3 rounded-lg border bg-card p-4"><div><h3 className="text-lg font-semibold">Menu trigger</h3><p className="mt-1 text-sm text-muted-foreground">A trigger must expose the relationship to the menu it opens.</p></div><div className="rounded-md border bg-muted/20 p-4"><button type="button" aria-haspopup="menu" aria-expanded="true" aria-controls="button-example-menu" className="rounded-md border px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Actions ▾</button><div id="button-example-menu" role="menu" aria-label="Record actions" className="mt-2 w-40 rounded-md border bg-background p-1 text-sm"><a href="#button-edit" role="menuitem" className="block rounded px-2 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Edit</a><a href="#button-archive" role="menuitem" className="block rounded px-2 py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Archive</a></div></div><div className="space-y-1 text-sm"><p><strong>Use when:</strong> several related actions need to share one trigger.</p><p><strong>Avoid when:</strong> a single common action is hidden behind an extra menu.</p><p><strong>Check:</strong> expanded state, popup relationship, Arrow-key movement, Escape dismissal, and focus restoration are implemented together.</p></div></article>
                </div>
                  <p className="leading-7 text-muted-foreground">
